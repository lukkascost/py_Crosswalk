...
def measureTime10(img, bits=6):
    ...
    svm = cv2.SVM()
    svm.load("MODEL_M14_CM6_TH198_ATT10_ROUND_24.txt")
...
if gerarResultados:
    ...
    for k, m in enumerate([27, 14, 1]):
        ...
        for i in range(100):
            ...
            total_times.append(measureTime10(img, bits=5))
        total_times = np.array(total_times)
        imgs_times[k, :4] = np.min(total_times, axis=0)
        imgs_times[k, -1] = np.sum(np.min(total_times, axis=0))
        print imgs_times
        np.savetxt(PATH_TO_SAVE_RESULTS+"T2_M27,14,1_CM8b.txt", imgs_times, delimiter=',')
        np.savetxt(PATH_TO_SAVE_RESULTS+"T2_M27,14,1_CM8b_Line_{:03d}.txt".format(k), total_times, delimiter=',')
    ...
    for l, b in enumerate(range(2, 9)):
        ...
        print imgs_times
        np.savetxt(PATH_TO_SAVE_RESULTS+"T1_M1_CM2-8b.txt", imgs_times, delimiter=',')
        np.savetxt(PATH_TO_SAVE_RESULTS+"T1_M1_CM2-8b_Line_{:03d}.txt".format(l), total_times, delimiter=',')
